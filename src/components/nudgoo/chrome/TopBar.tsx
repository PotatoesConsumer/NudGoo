...
export function TopBar({ v }: { v: VM }) {
  return (
    <div style={css("height:58px;flex:0 0 58px;display:flex;align-items:center;gap:10px;padding:0 16px;background:var(--canvas);border-bottom:1px solid var(--hairline-soft);z-index:4")}>
      ...
      <button style={css("flex:0 0 40px;width:40px;height:40px;border-radius:12px;border:0;background:var(--surface-raised);display:flex;align-items:center;justify-content:center;cursor:pointer;position:relative")}>
        <i className="ph-fill ph-bell" style={css("font-size:19px;color:var(--ink-secondary)")} />
        <span style={css("position:absolute;top:9px;right:9px;width:8px;height:8px;border-radius:50%;background:var(--error);border:1.5px solid var(--surface-raised)")} />
      </button>
